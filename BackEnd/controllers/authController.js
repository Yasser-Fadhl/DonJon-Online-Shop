...
exports.loginUser=catchAsyncErorrs(async(req, res, next)=>{
 ...
 return next(new ErrorHandler('Invalid email or password',401))
   ...
   if (!isPaswordMatched) {
    return next(new ErrorHandler('Invalid password or Email 2',401))
}
   sendToken(user,200,res);
...
exports.updatePassword= catchAsyncErorrs(async(req,res, next) => {
   ...
   if(!req.body.newPassword === req.body.confirmedPassword) next(new ErrorHandler('Passwords do not match'),400)
...
exports.resetPassword=catchAsyncErorrs(async(req, res, next) => {
   ...
   if (req.body.password!== req.body.confirmedPassword) {return next(new ErrorHandler("Passwords do not match",400));}
...
exports.getUser = catchAsyncErorrs(async(req, res, next) => {
   const user = await User.findById(req.params.id);
      if (!user) return next(new ErrorHandler(`User with ID ${req.params.id} doesn't exist`,500));
